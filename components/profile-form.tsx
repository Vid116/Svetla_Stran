  ...
  return (
    <>
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <p className="mt-2 text-xs text-muted-foreground/60">Ime nastavlja administrator.</p>
      </div>
...
function UsernameForm({ currentUsername }: { currentUsername: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (trimmed.length < 3) { setError("Uporabnisko ime mora imeti vsaj 3 znake"); return; }
    if (!/^[a-z0-9._-]+$/.test(trimmed)) { setError("Samo male crke, stevilke, pike, podcrke in pomisljaji"); return; }
    if (trimmed === currentUsername) { setError("Uporabnisko ime je enako"); return; }
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json(); throw new Error(data.error || "Napaka"); }
      setSuccess(true);
      setTimeout(() => { window.location.href = "/prijava"; }, 1500);
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-border bg-card p-6 space-y-4">
      ...
      {success && <div className="rounded-lg bg-nature/10 p-3 text-xs text-nature">Uporabnisko ime spremenjeno. Preusmerja na prijavo ...</div>}
