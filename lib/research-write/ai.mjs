...
/**
 * Multi-turn Claude call with FetchArticleText only (no WebSearch).
 * Used for spot-checking — re-reads known URLs to verify facts independently.
 */
export async function askClaudeWithFetch(systemPrompt, userMessage, maxTurns = 5, phase = 'fetch') {
  let result = '';
  let resultMsg = null;
  let toolCalls = 0;
  ...
  for await (const msg of query({
    ...
    options: {
      ...
      maxTokens: 16000,
      tools: [],
      allowedTools: ['mcp__svetla-tools__FetchArticleText'],
    ...
    if (msg.type === 'tool_progress') {
      toolCalls++;
      process.stderr.write(`[SpotCheck] Fetch: ${msg.tool_name} (${msg.elapsed_time_seconds?.toFixed(1)}s)\n`);
    }
  }
  ...
  if (resultMsg) {
    tokenTracker.record(phase, resultMsg);
    const p = tokenTracker.phases[phase];
    process.stderr.write(`[Tokens] ${phase}: ${p.inputTokens + p.outputTokens} tokens (in:${p.inputTokens} out:${p.outputTokens}) | ${p.turns} turns | ${toolCalls} fetches\n`);
  }

  if (!result) {
    process.stderr.write(`[askClaudeWithFetch] WARNING: empty response after ${toolCalls} fetches\n`);
  }
  return result;
}
