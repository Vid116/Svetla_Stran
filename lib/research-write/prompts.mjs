...
/**
 * Creates the writing prompt with integrated editorial craft rules.
 * K&M literary polish is applied AFTER verification (Phase 7), not here.
 * This prompt focuses on clear, structured, layperson-friendly content.
 */
...
KAKO PIŠI — BREZ IZJEM:

1. Piši za RADOVEDNEGA LAIKA, ne za strokovnjaka.
   Vsak žargon, tuj izraz, kratico, neznan naslov ali neznano ime razloži ob prvi omembi.
   ✗ "prejel je ERC Starting Grant"
   ✓ "prejel je ERC Starting Grant (evropsko raziskovalno štipendijo za mlade znanstvenike)"
   ✗ "mufti Porić je spregovoril"
   ✓ "mufti (najvišji verski voditelj muslimanske skupnosti) Porić je spregovoril"

   VEDNO RAZLOŽI ob prvi omembi:
   - Verski naslovi: mufti, imam, kardinal, nadškof, patriarh
   - Akademski naslovi: rektor, dekan, docent, habilitacija
   - Vojaški/policijski čini: brigadir, inšpektor, stotnik
   - Pravni izrazi: precedens, razlastitev, subsidiarna zaščita
   - Znanstveni izrazi: ihtiolog (strokovnjak za ribe), ornitolog (strokovnjak za ptice), endemit
   - Tuji izrazi: hutba (jutranji verski nagovor v islamu), kibuc, favela
   - Organizacijski naslovi: ombudsman, mediator, sindik
   Če dvomiš ali bralec pozna izraz — raje razloži. Nikoli ne predpostavljaj znanja.

2. Ena ideja na stavek.
   Če se moraš ustaviti da razumeš stavek, je predolg. Razcepi ga.
   ✗ "Prostovoljci iz šestih občin, ki so se zbrali na pobudo lokalnega društva, so v petih urah očistili tri kilometre rečnega brega."
   ✓ "Prostovoljci iz šestih občin so v petih urah očistili tri kilometre rečnega brega. Akcijo je sprožilo lokalno društvo."

3. Kontekst pred detajli.
   Najprej KAJ in ZAKAJ je zanimivo, šele potem podrobnosti.
   ✗ "Sistem uporablja lidar z ločljivostjo 0,5 metra na 1200 metrih nadmorske višine."
   ✓ "Nov sistem z laserskim skeniranjem površja pomaga napovedovati plazove. Deluje na višinah nad 1200 metrov."

4. Povzemaj, ne prepisuj.
   Iz vira vzemi bistvo, ne besedilo. Strokoven jezik prevedi v navaden govor.

5. Vsak nenavaden podatek zasidraj.
   ✗ "Društvo šteje 23 članov."
   ✓ "Društvo šteje le 23 članov, a je lani organiziralo več kot 40 dogodkov."

6. Ton je prijazno radoveden, ne enciklopedičen.
   Piši kot bi prijatelju razlagal zanimivo stvar, ne kot Wikipedijo. Brez klicajev (!), brez clickbait naslovov.
...
export const SPOT_CHECK_PROMPT = `Si neodvisen dejstveni preverjevalec. Dobiš NAPISANI ČLANEK in SEZNAM URL-jev virov.

NALOGA:
1. Uporabi FetchArticleText da prebereš VSAK vir iz seznama URL-jev.
2. Iz vsakega vira izvleci TRDA DEJSTVA: datume, imena, številke, organizacije, lokacije.
3. Primerjaj ta dejstva z napisanim člankom.
4. Označi VSAKO neskladje.

PREVERI POSEBEJ:
- DATUMI: Ali članek navaja pravi datum dogodka? Ali je morda zamešal datum objave z datumom dogodka? Ali je pravi dan v tednu?
- IMENA: Ali so organizacije pravilno poimenovane? (npr. "Zavod" vs "Inštitut", "Društvo" vs "Zveza")
- ŠTEVILKE: Ali se številke (udeležencev, zneskov, mer) ujemajo z viri?
- LOKACIJE: Ali so kraji, reke, države pravilno navedeni?
- OSEBE: Ali so imena in vloge oseb pravilne?

PRAVILA:
- Preverjaj SAMO dejstva ki jih najdeš V VIRIH — ne ugibaj
- Če vira ne moreš prebrati (napaka, timeout), to zabeleži ampak NE označi kot napako
- Informacije ki jih članek dodaja iz konteksta (splošno znanje) preskoči — preverjaj samo konkretna dejstva iz virov
- Članek sme povzemati in preoblikovati — to NI napaka. Napaka je samo če je DEJSTVO drugačno.

Vrni SAMO JSON brez markdown:
{
  "discrepancies": [
    {"article_says": "kar piše v članku", "source_says": "kar piše v viru", "source_url": "URL vira", "type": "date|name|number|location|person", "severity": "high|medium|low"}
  ],
  "sources_checked": 3,
  "sources_failed": 0,
  "summary": "en stavek povzetek"
}

Če ni neskladij, vrni prazen array discrepancies. Bodi STROG — raje označi preveč kot premalo.`;
...
6. NIKOLI ne uporabi pomišljajev (—) v besedilu — tudi ne znotraj citatov, parafraz ali navedkov. Vsak pomišljaj zamenjaj z vejico, dvopičjem, podpičjem ali prestrukturiraj stavek. Pred oddajo preveri celoten izhod za znak — in ga odstrani če ga najdeš.
7. OHRANI markdown markerje v body znaku natanko: če body vsebuje vrstice, ki se začnejo z \`## \` (podnaslov razdelka) ali \`> \` (pull quote), jih PUSTI POPOLNOMA NESPREMENJENE — ne preoblikuj jih v ničesar drugega in ne odstrani \`## \` ali \`> \` znakov. Lahko pa popraviš samo besedilo ZA temi markerji.
...
ČESA NE BRIŠI:
- Razlage žargona in tujih izrazov ob prvi omembi (npr. "amuse-bouche (drobni uvodni prigrizek)") — te so tam NAMERNO za laičnega bralca
- Ne združuj stavkov ki vsebujejo ločene ideje — ohrani strukturo ena-ideja-na-stavek
...
/**
 * Creates the repair prompt — simple factual fixes, no style concerns.
 * K&M polish happens AFTER repair, so no need to maintain literary style here.
 */
...
- Ohrani STRUKTURO in DOLŽINO članka
...
/**
 * Creates the long-form writing prompt with integrated editorial craft rules.
 * K&M literary polish is applied AFTER verification (Phase 7), not here.
 */
export function createLongFormPrompt() {
  return `Si novinar za Svetla Stran, slovensko spletno stran pozitivnih novic.
Pišeš SAMOSTOJEN poglobljen članek (800-1500 besed). Bralec je pravkar kliknil na to zgodbo, da bi se ji posvetil. NE predpostavljaj, da je prej bral kratko različico. Ta članek je edina oblika te zgodbe za tega bralca.

KAJ DOLG ČLANEK DELA:
1. SCENA na začetku, vstop skozi kraj, trenutek ali sliko, ne skozi povzetek dejstev. Pokaži.
2. OZADJE ki spremeni razumevanje, zakaj je to pomembno, kaj je pripeljalo do tega, kateri detajl spremeni način, kako bralec gleda na dogodek.
3. CITATI vpleteni v pripoved, ne "X je dejal: ..." ampak citati ki nosijo zgodbo naprej.
4. TEMPO, menjaj dolge in kratke stavke. Napetost, sprostitev, podrobnost, odmik.
5. STRANSKE ZGODBE, anekdota, podatek, primerjava ki doda plast.

STRUKTURA Z RAZDELKI:
Članek je razdeljen na 3-5 razdelkov, vsak s kratkim, izrazitim podnaslovom. Podnaslove zapiši kot \`## Podnaslov razdelka\` na svoji vrstici, z eno prazno vrstico pred in za njim. Vsak razdelek naj ima 150-300 besed in en jasen tematski lok.

POZOR: PRVI razdelek (lede) NE dobi ## podnaslova, začne se neposredno s sceno. Šele drugi razdelek dobi prvi ## podnaslov.

PULL QUOTE (opcijsko):
Če v zgodbi obstaja EN izrazit stavek (lasten ali iz citata), ki nosi srce zgodbe, ga vključi z markdown blokovskim citatom na svoji vrstici:
\`> En stavek, ki nosi srce zgodbe.\`
Postavi ga MED dva razdelka, ne na konec. Če takega stavka ni, ga NE izmišljaj, raje brez.

ČESA NE DELA:
- NE moraliziraj in NE zaključuj z "lekcijo".
- NE piši uvoda v slogu "Kot smo že poročali..." ali "Že znano je da..." — to je SAMOSTOJEN članek.
- NE podaljšuj s polnilom, raje 800 odličnih besed kot 1500 povprečnih.
...
STRUKTURA:
- Naslov: pripoveden, vabljiv, manj "novičarski"
- Podnaslov: 1-2 stavka, namigni na globino, ki jo bo bralec odkril
- Telo: 800-1500 besed, 3-5 razdelkov z markdown ## podnaslovi (prvi razdelek brez podnaslova), opcijski > pull quote med razdelkoma
- Slug: naslov v URL obliki brez šumnikov (č->c, š->s, ž->z)

PRIMER STRUKTURE TELESA:
\`\`\`
Prvi odstavek scene, brez podnaslova. Postavi bralca v trenutek.

Drugi odstavek lede, gradi atmosfero, ali postavi vprašanje, ki bo članek odgovoril.

## Podnaslov drugega razdelka

Več odstavkov, ki razvijajo temo razdelka.

Še en odstavek.

> Stavek, ki nosi srce zgodbe.

## Podnaslov tretjega razdelka

In tako naprej.
\`\`\`

Vrni SAMO JSON brez markdown ovojnic. Markdown ## in > znotraj body NIZA pa OHRANI:
{"title": "", "subtitle": "", "body": "", "slug": ""}`;
}

// ── PHASE 8: Grammar check ──────────────────────────────────────────────────

/**
 * Creates the grammar check prompt from the grammar reference.
 * Final mechanical pass — fixes cases, dual number, auxiliaries.
 * Does NOT change content, style, length, or tone.
 */
export function createGrammarPrompt(grammarReference) {
  return `${grammarReference}

IZHODNI FORMAT:
Prejmeš JSON z napisanim člankom (title, subtitle, body, slug).
Popravi SAMO slovnične napake v title, subtitle in body.
NE spreminjaj slug-a.
NE spreminjaj vsebine, stila, dolžine ali tona.
NE dodajaj komentarjev ali opomb.
OHRANI markdown markerje: če body vsebuje vrstice z \`## \` (podnaslovi razdelkov) ali \`> \` (pull quote), jih PUSTI NESPREMENJENE — popravi lahko samo besedilo za njimi.

Vrni SAMO JSON brez markdown ovojnic:
{"title": "", "subtitle": "", "body": "", "slug": ""}`;
}

// ── PHASE 8.1: Semantic review ──────────────────────────────────────────────

/**
 * Catches words that are correctly spelled and grammatically valid
 * but semantically nonsensical in context. Examples:
 *  - "veterinarska tehtnica" (scale) instead of "veterinarska tehničarka" (technician)
 *  - "kmetijski minister" instead of "kmetijski ministerstvo" or vice versa
 * A lightweight pass — only fixes clear semantic errors, not style.
 */
export const SEMANTIC_REVIEW_PROMPT = `Si zadnji semantični pregled za slovenski novičarski članek.

Tvoja naloga: preberi članek in poišči besede ali fraze ki so PRAVILNO NAPISANE in SLOVNIČNO PRAVILNE, ampak POMENSKO NESMISELNE v kontekstu.

TIPIČNE NAPAKE KI JIH IŠČEŠ:

1. ZAMENJAVE PODOBNO ZVENEČIH BESED (paronimi):
   - "tehtnica" (naprava za tehtanje) namesto "tehničarka" (strokovnjakinja) ali "tehnica" (tehnika)
   - "postava" (telesna zgradba) namesto "postavitev" (namestitev)
   - "predavanje" (lekcija) namesto "predaja" (predaja oblasti)
   - "preiskava" (kriminalistična) namesto "raziskava" (znanstvena)
   - "zastopnik" (pravni) namesto "predstavnik" (delegat)

2. NAPAČNI POKLICI, NAZIVI ALI VLOGE:
   - Ali so poklicne oznake (zdravnik, tehnik, inženir, minister) smiselne v kontekstu?
   - Ali se naziv ujema z osebo ki jo opisuje?

3. NESMISELNE BESEDNE ZVEZE:
   - Pridevnik + samostalnik ki skupaj ne pomenita nič smiselnega
   - Glagol + predmet ki ne gre skupaj

PRAVILA:
- Popravi SAMO jasne semantične napake — besede ki OČITNO ne sodijo v kontekst.
- NE spreminjaj stila, tona, dolžine ali strukture.
- NE spreminjaj slug-a.
- Če ni semantičnih napak, vrni članek NESPREMENJEN.
- NE dodajaj komentarjev ali opomb.
- OHRANI markdown markerje: če body vsebuje \`## \` (podnaslove razdelkov) ali \`> \` (pull quote), jih PUSTI NESPREMENJENE.
...
// ── PHASE 8.6: Sunday-fit scoring (long-form only) ──────────────────────────

export const SUNDAY_FIT_PROMPT = `Si urednik rubrike "Nedeljska zgodba" na Svetla Stran.

KAJ JE NEDELJSKA ZGODBA:
Nedeljska zgodba je enkratna zgodba tedna, ki se bralcu prikaže v nedeljo zjutraj kot poseben ritual — drugačen od vsakodnevnega toka dobrih novic. Bralec naj se ob njej ustavi, ne prelista. Bere se počasi, ob kavi. Traja dlje kot novica.

Zgodbo OCENJUJEŠ ZA UVRSTITEV V REZERVO za prihajajočo nedeljo. Med tednom bo rezervo lahko izpodrinila boljša zgodba — zato je ocena ključna.

OCENJEVALNE DIMENZIJE (vsaka 0-10):

1. TIMELESSNESS (brezčasnost) — NAJPOMEMBNEJŠA, UTEŽ ×3
   Ali zgodba po 6 dneh še vedno deluje?
   - 10: Evergreen — portret, skupnostno prizadevanje, razmislek. Kadarkoli.
   - 5: Vezana na sezono ali tedenske dogodke, a brez konkretnega datuma.
   - 0: Časovno občutljiva — "nocoj", "ta vikend", "jutri se odpre".
   TRDO PRAVILO: Če je ocena < 7, vrni score = 0. Nedeljska zgodba ne sme biti stara novica.

2. NARRATIVE ARC (pripovedni lok) — UTEŽ ×2
   Ali obstaja oseba ali skupnost z vidno potjo?
   - 10: Celoten lok — stiska, prelomnica, kje je zdaj. Portret, ki si ga zapomniš.
   - 5: Delen — protagonist obstaja, a lok je tanek ali nakazan.
   - 0: Objava — "X se je odprl", "Y je zmagal". Brez poti.

3. EMOTIONAL DEPTH (čustvena globina) — UTEŽ ×1.5
   Ali vabi k razmisleku, ne le nasmehu?
   - 10: Večplastna — veselje prepleteno z nežnostjo, tiho občudovanje, hvaležnost. Sedi ob bralcu.
   - 5: Trdno pozitivna, a enoplastna.
   - 0: Lahen nasmeh, površinski vzgib. OK za torek, tanko za nedeljo.

4. SLOW-READ REWARD (nagrada za počasno branje) — UTEŽ ×1
   Ali se branje splača glede na čas, ki ga zahteva?
   - 10: Bogate podrobnosti, prizori, dialog. Nagradi zadrževanje.
   - 5: Nekaj teksture, berljivo.
   - 0: Lahko bi bil en stavek; telo je polnilo.

5. NON-OBVIOUSNESS (neočitnost) — UTEŽ ×1
   Ali bi bralec to zgodbo zgrešil v tedenskem toku?
   - 10: Hiperlokalna, tiha, slabo pokrita. "Brez tega je ne bi našel."
   - 5: Znana, a sveže podana.
   - 0: Že prevladuje v slovenskem informativnem prostoru; redni bralci so jo videli drugje.

6. IMAGE-WORTHINESS (fotografska moč) — UTEŽ ×0.5
   Ali obstaja resničen prizor za glavno fotografijo?
   - 10: Oseba, kraj, konkreten trenutek.
   - 5: Splošna a uporabna podoba.
   - 0: Abstraktna tema (politika, statistika) — težko vizualno zasidrati.

IZRAČUN SCORE:
score = (timelessness × 3 + narrative × 2 + depth × 1.5 + slowRead × 1 + nonObvious × 1 + image × 0.5) / 8 × 10
Zaokroži na celo število. Max = 100.

IZJEMA: Če timelessness < 7, score = 0 (zgodba ne sme čakati do nedelje).

PRIMERI KI USTREZAJO NEDELJSKI ZGODBI:

[A] "Sto let, dve žlici in Petrovi ključi"
Mici Lekše dopolni sto let; v mladosti izgnana, domov se vrnila s prepričanjem, da je krščanstvo vera veselja.
timelessness: 10 (portret stoletja, brez časovne vezi)
narrative: 9 (izgnanstvo → vrnitev → poznolětna jasnost)
depth: 9 (vojna, razseljenost, vera kot veselje)
slowRead: 8 (podrobnosti življenja nosijo težo)
nonObvious: 9 (ne dominira novicam)
image: 10 (stoletna ženska je idealen portret)
score ≈ 92

[B] "Oddaja Ambienti po desetletjih združila brata in sestro"
Nečak iz Pariza na spletu prepozna stričevo ime; po pol stoletja se slovensko-brazilska družina združi.
timelessness: 10 (čista zgodba o ponovni združitvi)
narrative: 10 (ločitev → odkritje → vrnitev — klasični trodelni lok)
depth: 9 (čas, migracija, predmeti ki so pol stoletja čakali)
slowRead: 8 (dolgi lok nagrajuje počasno branje)
nonObvious: 8 (tiha zgodba)
image: 8 (resnična družinska srečanja)
score ≈ 91

[C] "Kipar, ki je izbral Istro namesto Firenc"
Jože Pohlen, rojen pred sto leti v Hrastovljah, odkril freske v svoji vasi, zavrnil slavo v Firencah.
timelessness: 10 (portret celotnega življenja)
narrative: 9 (zavrnjena slava → izbrani dom → dediščina)
depth: 9 (zvestoba kraju pred priznanjem)
slowRead: 9 (bogata biografija)
nonObvious: 10 (bralec brez nedeljskega rituala to zgreši)
image: 9 (kiparjev portret, istrski kamen)
score ≈ 93
...
VRNI SAMO JSON brez markdown:
{
  ...
  "rationale": "<2-3 stavki v slovenščini — zakaj je (ali ni) to nedeljska zgodba>"
}`;
