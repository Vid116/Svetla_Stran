...
async function main() {
  ...
  log(`Tokens: ${tokens.totalTokens} total (input: ${tokens.inputTokens}, output: ${tokens.outputTokens}) | ${tokens.webSearchRequests} web searches | $${tokens.costUSD?.toFixed(4)} USD`);
  for (const [phase, data] of Object.entries(tokens.phases)) {
    log(`  ${phase}: ${data.inputTokens + data.outputTokens} tokens | ${data.webSearchRequests || 0} searches | ${data.turns} turns | ${data.models?.join(', ') || '?'}`);
  }
