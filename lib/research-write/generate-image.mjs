...
export async function generateArticleImage(title, body, category, slug, sourceUrl = null) {
  ...
  // Step 1.5: If Claude wants a reference photo, fetch og:image from source NOW
  let refImage = null;
  if (useReference && sourceUrl) {
    refImage = await fetchReferenceImage(sourceUrl);
  }
  ...
  // Try Nano Banana 2 — pass reference if Claude said yes and we fetched one
