...
export async function generateArticleImage(title, body, category, slug, heroImage = null) {
  ...
  if (heroImage) log(`Reference image: ${heroImage.slice(0, 80)}...`);

  // Step 2: Try image generation — priority order:
  // 1. Nano Banana 2 (best quality, supports reference photos, cheap batch)
  // 2. Cloudflare FLUX (fast, 50/month free)
  // 3. HuggingFace FLUX (slower, daily reset free)
  let imageBuffer = null;

  // Try Nano Banana 2 (with reference photo if available for person stories)
  const useReference = heroImage && ['JUNAKI', 'SPORT'].includes(category);
  imageBuffer = await tryNanoBanana(fullPrompt, useReference ? heroImage : null);
