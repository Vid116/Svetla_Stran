...
export async function generateArticleImage(title, body, category, slug, sourceUrl = null) {
  ...
  // Step 4: Generate BOTH images in parallel (saves ~30s wall time)
  const [wcBuf, photoBuf] = await Promise.all([
    tryNanoBanana(watercolorPrompt, refImageUrl, 'watercolor').catch(err => {
      log(`Watercolor gen failed: ${err.message}`); return null;
    }),
    tryNanoBanana(photoPrompt, refImageUrl, 'photo').catch(err => {
      log(`Photo gen failed: ${err.message}`); return null;
    }),
  ]);

  // Step 5: Upload BOTH to R2 in parallel
  const [wcUpload, photoUpload] = await Promise.all([
    wcBuf ? uploadToR2(wcBuf, slug, 'wc') : Promise.resolve(null),
    photoBuf ? uploadToR2(photoBuf, slug, 'photo') : Promise.resolve(null),
  ]);

  const watercolor = wcUpload?.imageUrl ? { url: wcUpload.imageUrl, prompt: watercolorPrompt } : null;
  const photo      = photoUpload?.imageUrl ? { url: photoUpload.imageUrl, prompt: photoPrompt } : null;
  ...
  const imagePrompt = chosen?.prompt || (watercolor ? watercolorPrompt : photoPrompt);

  if (watercolor && photo) log('✓ Both variants generated');
  else if (chosen) log(`⚠ Only ${watercolor ? 'watercolor' : 'photo'} variant generated`);
  else log('✗ Both variants failed');
