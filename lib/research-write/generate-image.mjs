 ...
 * 3. Cloudflare R2 — hosts the result
 */
...
const STYLE_SUFFIX = ', wide landscape format, main subject placed in the vertical and horizontal center of the frame, full-bleed edge-to-edge composition, color and texture extend all the way to all four edges of the frame, no white margins, no paper borders, no vignette, no unpainted areas. No text, no words, no letters, no writing, no numbers.';
...
export async function generateArticleImage(title, body, category, slug, sourceUrl = null) {
  ...
  // Step 1.5: If Claude wants a reference, get a person description
  // Priority: og:image from source → Wikipedia photo → give up
  let personDescription = null;
  let refImageUrl = null;
  if (useReference && sourceUrl) {
    // Try 1: og:image from the source article
    refImageUrl = await fetchReferenceImage(sourceUrl);
    if (refImageUrl) {
      personDescription = await describePersonFromPhoto(refImageUrl);
    }

    // Try 2: Wikipedia — extract person's name, find their wiki photo
  ...
  // Step 2: Generate image with Nano Banana 2 (Gemini)
  ...
  return await uploadToR2(imageBuffer, slug, fullPrompt);
}
...
function extractPersonName(title, body) {
  // Look for capitalized two-word names in the title first (most reliable)
  // Slovenian names: "Tadej Pogačar", "Luka Dončić", "Babica Manja"
  ...
  // Fallback: first capitalized name in body
...
async function fetchWikipediaPhoto(personName) {
  try {
    ...
    // Try Slovenian Wikipedia first, then English
...
async function uploadToR2(imageBuffer, slug, fullPrompt) {
  try {
    log(`Uploading ${(imageBuffer.length / 1024).toFixed(0)}KB image to R2...`);
    ...
    log(`✓ Image uploaded: ${publicUrl.slice(0, 80)}...`);
    return { imageUrl: publicUrl, imagePrompt: fullPrompt };
  } catch (err) {
