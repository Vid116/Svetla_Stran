 ...
 */
export async function generateArticleImage(title, body, category, slug, heroImage = null) {
  ...
  log(`Reference photo: ${useReference ? 'YES (Claude decided)' : 'NO'}`);
  if (heroImage) log(`Available reference: ${heroImage.slice(0, 80)}...`);

  // Step 2: Try image generation — priority order:
  // 1. Nano Banana 2 (best quality, supports reference photos)
  // 2. Cloudflare FLUX (fast, 50/month free)
  // 3. HuggingFace FLUX (slower, daily reset free)
  let imageBuffer = null;

  // Try Nano Banana 2 — pass reference only if Claude said yes AND we have one
  const refImage = (useReference && heroImage) ? heroImage : null;
  imageBuffer = await tryNanoBanana(fullPrompt, refImage);
